...
export function AcademicPromptPicker({ prompts, disabled, onReplace, onAppend }: AcademicPromptPickerProps) {
    ...
    return (
        <div className='space-y-2 rounded-xl border border-border/70 bg-background/45 p-2.5'>
            ...
            <Select value={selectedTitle} onValueChange={setSelectedTitle} disabled={disabled}>
                <SelectTrigger className='h-9 rounded-lg border-border bg-background/70 text-foreground'>
            ...
            <div className='rounded-lg border border-border/60 bg-card/50 p-2.5'>
                <div className='truncate text-sm font-medium text-foreground'>{selectedPrompt.title}</div>
                {selectedPrompt.description && (
                    <p className='mt-1 line-clamp-2 text-xs leading-snug text-muted-foreground'>{selectedPrompt.description}</p>
                )}
            ...
            <div className='grid grid-cols-2 gap-2'>
                <Button type='button' variant='outline' size='sm' disabled={disabled} onClick={() => onReplace(selectedPrompt)} className='h-8 rounded-lg'>
                    <Replace className='mr-2 h-3.5 w-3.5' />
                    覆盖提示词
                </Button>
                <Button type='button' size='sm' disabled={disabled} onClick={() => onAppend(selectedPrompt)} className='h-8 rounded-lg'>
