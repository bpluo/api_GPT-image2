...
export function validateGptImage2Size(width: number, height: number): SizeValidation {
    if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
        return { valid: false, reason: '宽度和高度必须大于 0。' };
    }
    ...
    if (width > GPT_IMAGE_2_MAX_EDGE || height > GPT_IMAGE_2_MAX_EDGE) {
        return { valid: false, reason: `单边最长为 ${GPT_IMAGE_2_MAX_EDGE} 像素。` };
    }
    const long = Math.max(width, height);
    const short = Math.min(width, height);
    if (long / short > GPT_IMAGE_2_MAX_ASPECT) {
        return { valid: false, reason: `长边与短边之比不能超过 ${GPT_IMAGE_2_MAX_ASPECT}:1。` };
    }
    const pixels = width * height;
    if (pixels < GPT_IMAGE_2_MIN_PIXELS) {
        return { valid: false, reason: `总像素不能少于 ${GPT_IMAGE_2_MIN_PIXELS.toLocaleString()}，请增大尺寸。` };
    }
    if (pixels > GPT_IMAGE_2_MAX_PIXELS) {
        return { valid: false, reason: `总像素不能超过 ${GPT_IMAGE_2_MAX_PIXELS.toLocaleString()}，请减小尺寸。` };
    }
    return { valid: true };
}
